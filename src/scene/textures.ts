...
/**
 * "DO IT FOR HER" memo. Abstract pastel comic-panel collage — coloured panels
 * and photo-like rectangles only, no characters — with a white sign band.
 */
export function memoPaper(): THREE.CanvasTexture {
  return makeTexture(
    "memo-paper",
    512,
    340,
    (ctx, W, H) => {
      ...
      // irregular collage grid of comic panels
      const cols = [0, 0.34, 0.63, 1.0];
      const rows = [0, 0.3, 0.58, 1.0];
      for (let r = 0; r < 3; r++) {
        for (let c = 0; c < 3; c++) {
          const x = cols[c] * W + 7;
          const y = rows[r] * H + 7;
          const w = (cols[c + 1] - cols[c]) * W - 14;
          const h = (rows[r + 1] - rows[r]) * H - 14;
          const tilt = (rnd() - 0.5) * 0.06;
          ctx.save();
          ctx.translate(x + w / 2, y + h / 2);
          ctx.rotate(tilt);
          ctx.fillStyle = pastels[(r * 3 + c + 2) % pastels.length];
          ctx.fillRect(-w / 2, -h / 2, w, h);

          // interior blocking: soft blobs + a photo-like rectangle
          ctx.globalAlpha = 0.45;
          for (let b = 0; b < 3; b++) {
            ctx.fillStyle = pastels[Math.floor(rnd() * pastels.length)];
            ctx.beginPath();
            ctx.ellipse(
              (rnd() - 0.5) * w * 0.6,
              (rnd() - 0.5) * h * 0.6,
          ...
          if (rnd() > 0.4) {
            const pw = w * (0.3 + rnd() * 0.22);
            const ph = h * (0.26 + rnd() * 0.2);
            ctx.fillStyle = "rgba(255,255,255,0.85)";
            ctx.fillRect(-pw / 2 + (rnd() - 0.5) * w * 0.3, -ph / 2, pw, ph);
            ctx.fillStyle = "rgba(120,140,160,0.35)";
            ctx.fillRect(-pw / 2 + (rnd() - 0.5) * w * 0.3 + 3, -ph / 2 + 3, pw - 6, ph - 6);
          }
      ...
      // white sign band with the message
      ctx.save();
      ctx.translate(W / 2, H * 0.55);
      ctx.rotate(-0.018);
      const bw = W * 0.82;
      const bh = 74;
      ctx.shadowColor = "rgba(40,32,24,0.4)";
      ctx.shadowBlur = 12;
      ctx.shadowOffsetY = 4;
      ...
      ctx.textBaseline = "middle";
      ctx.letterSpacing = "2px";
      ctx.fillText("DO IT FOR HER", 0, 2);
      ctx.restore();

      // paper wear: creases, warm foxing, softened corners
      ctx.strokeStyle = "rgba(120,100,72,0.16)";
      ctx.lineWidth = 1.6;
      for (let i = 0; i < 5; i++) {
        ctx.beginPath();
        ctx.moveTo(rnd() * W, 0);
      ...
      for (let i = 0; i < 20; i++) {
        const g = ctx.createRadialGradient(rnd() * W, rnd() * H, 2, rnd() * W, rnd() * H, 30 + rnd() * 70);
        g.addColorStop(0, "rgba(150,120,80,0.09)");
        g.addColorStop(1, "rgba(150,120,80,0)");
        ctx.fillStyle = g;
        ctx.fillRect(0, 0, W, H);
      }
      const vig = ctx.createLinearGradient(0, 0, 0, H);
      vig.addColorStop(0, "rgba(90,72,48,0.12)");
      vig.addColorStop(0.3, "rgba(90,72,48,0)");
      vig.addColorStop(1, "rgba(90,72,48,0.14)");
      ctx.fillStyle = vig;
      ctx.fillRect(0, 0, W, H);
    },
    { clamp: true },
...
/** Slabbed trading card: white label band + red stripe + gold holo art. */
export function gradedCard(): THREE.CanvasTexture {
  return makeTexture(
    "graded-card",
    384,
    620,
    (ctx, W, H) => {
      ...
      ctx.fillRect(0, labelH - 3, W, 3);

      ctx.fillStyle = "#1a1a1a";
      ctx.font = `700 17px ${SANS}`;
      ctx.textBaseline = "alphabetic";
      ctx.fillText("2025 DADMON TCG", 14, 38);
      ctx.font = `600 15px ${SANS}`;
      ctx.fillStyle = "#3a3a3a";
      ctx.fillText("No. 1 Dad", 14, 60);

      ctx.textAlign = "right";
      ctx.fillStyle = "#c8102e";
      ctx.font = `800 19px ${SANS}`;
      ctx.fillText("GEM MINT 10", W - 14, 40);
      ctx.textAlign = "left";

      // barcode suggestion
      const bx = 14;
      const rndBar = prng(99);
      for (let i = 0; i < 58; i++) {
        const bw = rndBar() > 0.6 ? 2.6 : 1.2;
        ctx.fillStyle = "#1a1a1a";
        ctx.fillRect(bx + i * 3.1, 72, bw, 20);
      }
      ctx.textAlign = "right";
      ctx.font = `500 11px ${MONO}`;
      ctx.fillStyle = "#666";
      ctx.fillText("0713 4429 08", W - 14, 88);
      ctx.textAlign = "left";

      // card art: warm gold holo field
      const artY = labelH;
      const artH = H - labelH;
      const g = ctx.createLinearGradient(0, artY, W, H);
      g.addColorStop(0, "#f2c766");
      g.addColorStop(0.32, "#e8a63f");
      g.addColorStop(0.55, "#f7dfa0");
      g.addColorStop(0.78, "#d9922f");
      g.addColorStop(1, "#b8701f");
      ctx.fillStyle = g;
      ...
      ctx.clip();
      ctx.translate(W / 2, artY + artH / 2);
      ctx.rotate(-0.62);
      for (let i = -14; i < 14; i++) {
        ctx.fillStyle = i % 2 === 0 ? "rgba(255,255,255,0.13)" : "rgba(255,190,90,0.10)";
        ctx.fillRect(-500, i * 26, 1000, 12);
      }
      ctx.restore();

      // painterly abstraction of three figures — soft silhouettes, no faces
      ctx.save();
      ctx.beginPath();
      ctx.rect(0, artY, W, artH);
      ctx.clip();
      const figs = [
        { x: W * 0.28, y: artY + artH * 0.66, s: 1.0, c: "rgba(126,72,26,0.42)" },
        { x: W * 0.52, y: artY + artH * 0.58, s: 1.24, c: "rgba(96,54,18,0.5)" },
        { x: W * 0.74, y: artY + artH * 0.7, s: 0.82, c: "rgba(140,86,34,0.38)" },
      ];
      for (const f of figs) {
        ctx.fillStyle = f.c;
        // torso
        ctx.beginPath();
        ctx.ellipse(f.x, f.y, 34 * f.s, 58 * f.s, 0, 0, Math.PI * 2);
        ctx.fill();
        // head — a plain blob, deliberately featureless
        ctx.beginPath();
        ctx.ellipse(f.x, f.y - 72 * f.s, 21 * f.s, 23 * f.s, 0, 0, Math.PI * 2);
        ctx.fill();
      }
      // rim light behind the group
      const rim = ctx.createRadialGradient(W * 0.5, artY + artH * 0.42, 10, W * 0.5, artY + artH * 0.42, 190);
      rim.addColorStop(0, "rgba(255,248,220,0.6)");
      ...
      for (let i = 0; i < 420; i++) {
        const x = rnd() * W;
        const y = artY + rnd() * artH;
        const r = 0.6 + rnd() * 2.2;
        ctx.fillStyle = `rgba(255,255,240,${0.15 + rnd() * 0.55})`;
        ctx.beginPath();
        ctx.arc(x, y, r, 0, Math.PI * 2);
        ctx.fill();
      }

      // card border + caption plate
      ctx.strokeStyle = "rgba(120,78,24,0.65)";
      ctx.lineWidth = 8;
      ctx.strokeRect(4, artY + 4, W - 8, artH - 8);
      ctx.fillStyle = "rgba(255,250,232,0.86)";
      ctx.fillRect(24, H - 66, W - 48, 34);
      ctx.fillStyle = "#7a4b12";
      ctx.font = `700 18px ${SANS}`;
      ctx.textAlign = "center";
      ctx.letterSpacing = "2px";
      ctx.fillText("No. 1 DAD", W / 2, H - 42);
      ctx.textAlign = "left";
...
/** Grey felt letterboard face with grooves, proud white letters, drop shadows. */
export function letterboardFace(): THREE.CanvasTexture {
  return makeTexture(
    "letterboard-face",
    512,
    512,
    (ctx, W, H) => {
      ...
      for (let i = 0; i < 14000; i++) {
        const v = rnd();
        ctx.fillStyle = v > 0.5 ? "rgba(255,255,255,0.05)" : "rgba(40,44,50,0.06)";
      ...
      for (let r = 0; r <= rows; r++) {
        const y = r * rowH;
        ctx.fillStyle = "rgba(46,50,56,0.55)";
        ctx.fillRect(0, y - 2, W, 3);
        ctx.fillStyle = "rgba(190,196,202,0.22)";
        ctx.fillRect(0, y + 1, W, 1.4);
      }

      // letters, one line per groove row, left aligned
      const lines = ["WILL", "SOURCE", "FOR", "$$"];
      ctx.font = `700 44px ${SANS}`;
      ctx.textBaseline = "middle";
      ctx.letterSpacing = "3px";
      for (let i = 0; i < lines.length; i++) {
        const y = rowH * (3 + i) + rowH * 0.5;
        // per-letter so each gets its own cast shadow, like real board letters
        let x = 34;
        for (const ch of lines[i]) {
          const cw = ctx.measureText(ch).width;
          if (ch !== " ") {
            ctx.fillStyle = "rgba(28,32,38,0.5)";
            ctx.fillText(ch, x + 4, y + 5);
      ...
      // ambient occlusion into the frame rebate
      const vig = ctx.createRadialGradient(W / 2, H / 2, W * 0.3, W / 2, H / 2, W * 0.78);
      vig.addColorStop(0, "rgba(0,0,0,0)");
      vig.addColorStop(1, "rgba(20,24,30,0.35)");
      ctx.fillStyle = vig;
      ctx.fillRect(0, 0, W, H);
    },
    { clamp: true },
...
/** Warm out-of-focus festival scene. Abstract bokeh only — no faces. */
export function familyPhoto(): THREE.CanvasTexture {
  return makeTexture(
    "family-photo",
    512,
    384,
    (ctx, W, H) => {
      ...
      g.addColorStop(0.42, "#6b3a2c");
      g.addColorStop(0.72, "#c9702f");
      g.addColorStop(1, "#f0a94e");
      ctx.fillStyle = g;
      ctx.fillRect(0, 0, W, H);

      const warm = ["#ffd98a", "#ff9c5b", "#ffe9b8", "#ff7f6e", "#ffcf5e", "#f4b183", "#ffeed0"];
      for (let i = 0; i < 130; i++) {
        const x = rnd() * W;
        const y = rnd() * H * 0.95;
        const r = 6 + rnd() * 46;
      ...
      // soft foreground shapes reading as people, deliberately faceless blurs
      for (let i = 0; i < 4; i++) {
        const x = W * (0.14 + i * 0.24) + (rnd() - 0.5) * 30;
        ctx.fillStyle = `rgba(48,26,20,${0.28 + rnd() * 0.18})`;
        ctx.beginPath();
        ctx.ellipse(x, H * 0.92, 46 + rnd() * 22, 74 + rnd() * 26, 0, 0, Math.PI * 2);
        ctx.fill();
      }

      const vig = ctx.createRadialGradient(W / 2, H / 2, W * 0.2, W / 2, H / 2, W * 0.72);
      vig.addColorStop(0, "rgba(0,0,0,0)");
      vig.addColorStop(1, "rgba(24,10,6,0.42)");
      ctx.fillStyle = vig;
      ctx.fillRect(0, 0, W, H);
    },
    { clamp: true },
...
/** Kid-made ceramic tile: uneven cobalt glaze, hand-painted lowercase "f". */
export function ceramicTile(): THREE.CanvasTexture {
  return makeTexture(
    ...
    (ctx, W, H) => {
      const rnd = prng(4409);
      ctx.fillStyle = "#2a63d8";
      ctx.fillRect(0, 0, W, H);

      // brushwork: overlapping strokes at varying blue, deliberately uneven
      for (let i = 0; i < 190; i++) {
        const y = rnd() * H;
        const hue = 218 + (rnd() - 0.5) * 22;
        const sat = 68 + rnd() * 26;
        const lig = 38 + rnd() * 26;
        ctx.strokeStyle = hsl(hue, sat, lig, 0.16 + rnd() * 0.3);
        ctx.lineWidth = 12 + rnd() * 34;
        ctx.lineCap = "round";
        ctx.beginPath();
        ctx.moveTo(-20, y);
        ctx.bezierCurveTo(W * 0.3, y + (rnd() - 0.5) * 50, W * 0.7, y + (rnd() - 0.5) * 50, W + 20, y + (rnd() - 0.5) * 30);
        ctx.stroke();
      }

      // glaze pooling toward the edges
      const pool = ctx.createRadialGradient(W / 2, H / 2, W * 0.22, W / 2, H / 2, W * 0.72);
      pool.addColorStop(0, "rgba(96,158,255,0.28)");
      pool.addColorStop(1, "rgba(14,52,150,0.4)");
      ctx.fillStyle = pool;
      ctx.fillRect(0, 0, W, H);

      // hand-painted lowercase f — wobbly strokes, thick brush
      ctx.save();
      ...
      ctx.beginPath();
      ctx.moveTo(6, 152);
      ctx.bezierCurveTo(2, 40, -4, -46, 10, -104);
      ctx.bezierCurveTo(22, -150, 62, -156, 84, -138);
      ctx.stroke();
      ctx.lineWidth = 40;
      ctx.beginPath();
      ctx.moveTo(-70, -34);
      ctx.bezierCurveTo(-20, -42, 40, -40, 78, -46);
      ctx.stroke();
      // a slightly-off second pass so edges read hand-made, not vector
      ctx.globalAlpha = 0.55;
      ctx.strokeStyle = "#e8ecff";
      ctx.lineWidth = 34;
      ctx.beginPath();
      ctx.moveTo(10, 144);
      ctx.bezierCurveTo(4, 36, -1, -44, 14, -100);
      ctx.stroke();
      ctx.globalAlpha = 1;
      ctx.restore();

      // fired speckles + a couple of tiny glaze pinholes
      for (let i = 0; i < 900; i++) {
        ctx.fillStyle = rnd() > 0.5 ? "rgba(255,255,255,0.07)" : "rgba(0,10,50,0.12)";
        ctx.beginPath();
        ctx.arc(rnd() * W, rnd() * H, 0.8 + rnd() * 2.6, 0, Math.PI * 2);
        ctx.fill();
      }
    },
    { clamp: true },
...
/** Abstract chest badge for the droid: blue shield outline + tiny colour marks. */
export function droidBadge(): THREE.CanvasTexture {
  return makeTexture(
    "droid-badge",
    128,
    128,
    (ctx, W, H) => {
      ...
      ctx.moveTo(W * 0.5, 16);
      ctx.lineTo(W - 20, 36);
      ctx.lineTo(W - 20, H * 0.56);
      ctx.quadraticCurveTo(W - 20, H - 18, W * 0.5, H - 12);
      ctx.quadraticCurveTo(20, H - 18, 20, H * 0.56);
      ctx.lineTo(20, 36);
      ctx.closePath();
      ctx.stroke();
      const marks = ["#e8453c", "#f7b500", "#3ba55d", "#2f6bd8"];
      for (let i = 0; i < marks.length; i++) {
        ctx.fillStyle = marks[i];
        ctx.beginPath();
        ctx.arc(W * 0.5 + (i - 1.5) * 15, H * 0.52, 6.5, 0, Math.PI * 2);
        ctx.fill();
      }
    },
    { clamp: true },
